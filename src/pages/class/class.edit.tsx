import { Button, useToast } from '@/entities'
import { UploadInfo } from '@/features'
import { classStore, FILE_MAX_SIZE, globalStore, isEmpty, RESPONSE_OK, ROUTE_PATH, Server, STATUS_OK, userStore } from '@/shared'
import { useRouter } from 'next/navigation'
import { useEffect, useState } from 'react'
import Contents from './class.style'

const _ = () => {
	const router = useRouter()
	const { toast } = useToast()

	const { user } = userStore()
	const { setClassList } = classStore()
	const { setHeader, clearBackLogic } = globalStore()
	useEffect(() => {
		setHeader({
			type: 'sub',
			title: '반 정보',
			back: true,
		})
	}, [])

	const [file, setFile] = useState<File | null>(null)

	useEffect(() => {
		return () => clearBackLogic()
	}, [])

	const handleNextBtn = () => {
		if (isEmpty(file)) {
			return toast('파일을 등록해주세요')
		}

		const formData = new FormData()
		formData.append('excel', file as File)

		Server.post(`/api/daycare/class/upsert/${user.daycareId}`, formData).then(response => {
			const { data, status } = response
			if (status === RESPONSE_OK) {
				const { state, code, data: resData, message } = data
				if (state === STATUS_OK) {
					setClassList(resData)
					router.replace(ROUTE_PATH['21-03'])
				} else {
					toast(message)
				}
			}
		})
	}

	return (
		<Contents className='bg-white'>
			<div className='class-modify-wrap'>
				<UploadInfo txt={'반'} file={file} max={FILE_MAX_SIZE} onChange={setFile} noti={'반 정보 변경 시 선생님, 원아 정보를 다시 등록 해야 합니다.'} />
				<div className='btn-wrap'>
					<Button className='btn-type2 st1' disabled={isEmpty(file)} onClick={handleNextBtn}>
						<span>다음</span>
					</Button>
				</div>
			</div>
		</Contents>
	)
}

export default _
